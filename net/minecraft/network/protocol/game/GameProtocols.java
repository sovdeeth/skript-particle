...
public class GameProtocols {
    public static final CodecModifier<RegistryFriendlyByteBuf, ServerboundSetCreativeModeSlotPacket, GameProtocols.Context> HAS_INFINITE_MATERIALS = (codec, context) -> new StreamCodec<RegistryFriendlyByteBuf, ServerboundSetCreativeModeSlotPacket>(
        
    ) {
        @Override
        public ServerboundSetCreativeModeSlotPacket decode(RegistryFriendlyByteBuf buffer) {
            if (!context.hasInfiniteMaterials()) {
                throw new SkipPacketDecoderException("Not in creative mode");
            } else {
                return (ServerboundSetCreativeModeSlotPacket)codec.decode(buffer);
            }
        }

        @Override
        public void encode(RegistryFriendlyByteBuf buffer, ServerboundSetCreativeModeSlotPacket value) {
            if (!context.hasInfiniteMaterials()) {
    ...
    public static final UnboundProtocol<ServerGamePacketListener, RegistryFriendlyByteBuf, GameProtocols.Context> SERVERBOUND_TEMPLATE = ProtocolInfoBuilder.contextServerboundProtocol(
        ...
        protocolInfoBuilder -> protocolInfoBuilder.addPacket(
                GamePacketTypes.SERVERBOUND_ACCEPT_TELEPORTATION, ServerboundAcceptTeleportationPacket.STREAM_CODEC
            )
