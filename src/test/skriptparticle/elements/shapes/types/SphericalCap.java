...
public class SphericalCap extends Shape implements RadialShape {
    ...
    public Shape particleCount(int count) {
        this.particleDensity =  switch (style) {
            case Style.OUTLINE, Style.SURFACE -> Math.sqrt(2 * Math.PI * radius * radius * (1 - cutoffAngleCos) / count);
            case Style.FILL -> Math.cbrt(Math.PI / 3 * radius * radius * radius * (2 + cutoffAngleCos) * (1 - cutoffAngleCos) * (1 - cutoffAngleCos) / count);
        };
