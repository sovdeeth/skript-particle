...
public class ExprSphericalCap extends SimpleExpression<SphericalCap> {
    ...
    static {
        Skript.registerExpression(ExprSphericalCap.class, SphericalCap.class, ExpressionType.COMBINED, "[a] spherical cap (with|of) radius %number%[,| and] [cutoff] angle %number% [degrees|:radians]");
    }
    ...
    protected @Nullable SphericalCap[] get(Event event) {
        ...
        return new SphericalCap[]{new SphericalCap(radiusExpr.getSingle(event).doubleValue(), angle)};
    }
